...
-- All below was written by Michael Seman
...
ROUND(SUM(S.PointsPerGame*s.GamesPlayed)/SUM(S.GamesPlayed),1) as 'PPG'
FROM PlayerStats s JOIN Players P ON s.PlayerID = p.PlayerID
GROUP BY CONCAT(p.LastName, ', ', P.FirstName), Age, CONCAT(P.FirstName, ' ', P.LastName), S.PlayerID
GO


-- EXTRA QUERIES SECTION
select * from V_dbo_Full_Player_Stats
-- this queries were done by Michael Seman to take advantage of the view he created and to practice different SQL techniques.
...
WHILE @Counter <= @CounterEnd
	BEGIN
		...
		ELSE
			INSERT INTO ThreePointAgeRank ([PlayerID],[FullName],[NormalName],[Age],[GP],[MP],[FG%],[FGA],[3P%],[3PA],[2P%],[EFG%],[FT%],[FTA],[OReb],[DReb],[TotReb],[Asst],[Stl],[Blk],[TO],[PPG],[3ptRankingByAge])
...
-- A query using a CTE to compare players 3 point shooting percentage to the MAX of their Age group and the MIN of their Age group
WITH avg_age AS(
	SELECT Age, MAX([3P%]) as 'Max_3P_By_Age', MIN([3P%]) as 'Min_3P_By_Age'
	FROM V_dbo_Full_Player_Stats
	WHERE [3PA] >2 AND GP > 41
	GROUP BY age)
SELECT  D.NormalName, D.GP, D.AGE,D.[3PA], D.[3P%], A.[Max_3P_By_Age], A.[Min_3P_By_Age] FROM V_dbo_Full_Player_Stats D 
JOIN avg_age A
on D.age = A.age
WHERE [3PA] >2 AND GP > 41
ORDER BY D.Age 
